...
// CcToggle: 62px tall pill with a 36px circular icon ring on the left, two
...
// the accent (mirrors the CcToggle "on" look).
//
...
Rectangle {
    ...
    function statusText() {
        if (net.active)
            return "Connected";
        if (Networks.connectingSsid === net.ssid)
            return "Connecting…";
    ...
    ColumnLayout {
        ...
        Item {
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    implicitHeight: 36
                    radius: 18
                    color: item.net.active ? Theme.rgba("ffffff", Theme.activeTone === "light" ? 0.4 : 0.22) : Theme.fillStrong
        ...
        RowLayout {
            ...
            Rectangle {
                ...
                function go() {
                    if (item.net.active) {
                        Networks.disconnect(item.net.ssid);
                    } else {
                        Networks.connect(item.net.ssid, pwInput.text);
                    }
                    pwInput.text = "";
                    item.toggle();
                }
                ...
                Text {
                    id: actionText
                    anchors.centerIn: parent
                    text: Networks.connectingSsid === item.net.ssid ? "…" : (item.net.active ? "Disconnect" : "Connect")
                    font.family: Theme.fonts.sans
                    font.pixelSize: 12
