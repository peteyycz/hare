...
// One row in the BluetoothPanel list — sister to NetworkItem and styled the
// same way as a CcToggle: 62px tall pill with a circular icon ring + two
...
Rectangle {
    ...
    readonly property bool busy: Bluetooths.busyAddress !== "" && Bluetooths.busyAddress === (dev?.address ?? "")
            ...
            Rectangle {
                ...
                function go() {
                    if (!item.dev)
                        return;
                    if (item.dev.connected) {
                        Bluetooths.disconnect(item.dev);
                    } else {
                        Bluetooths.connect(item.dev);
                    }
                    item.toggle();
                }
